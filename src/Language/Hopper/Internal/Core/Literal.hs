...
newtype ConstrId  = ConstrId { unConstrId :: Text } deriving (Eq, Show,Typeable,Ord,Read,Data,Generic)

-- Primops for a language have names
newtype PrimOpId = PrimopId {unPrimopId :: Text}  deriving  (Eq, Show,Typeable,Ord,Read,Data,Generic)
